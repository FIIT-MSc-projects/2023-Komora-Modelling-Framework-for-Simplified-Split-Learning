...
def start_client(args: StartClientArguments):
    ...
    dist.init_process_group(backend='gloo', rank=args.get_rank(), world_size=args.get_world_size())
    rpc.init_rpc(name=f"{args.get_name()}{args.get_rank()}", rank=args.get_rank(), world_size=args.get_world_size())
    rpc.shutdown()